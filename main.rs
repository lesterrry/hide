...
fn decrypt(path: &fs::DirEntry, options: Option<DirectoryEnDeOptions>, index: usize) -> OperationStepResult {
	...
	if (!name.contains(CRPTFILEEXT) && !name.contains(CRPTFLDREXT)) || (path.path().extension().is_none() && !path.file_type().unwrap().is_dir())
	{
		...
		match size_from_file(&path.path()){
			Ok(result) => { len = result },
			Err(_) => { println!("\n{}Couldn't get size of {}{}", RED, name, RES); return OperationStepResult::Fail }
		}
		if options.is_some() && need_to_skip(options.unwrap(), Some(&name), len) {
			return OperationStepResult::Skip
		}
		match bytes_from_file(&path.path()){
			Ok(result) => { filecontent = result },
...
//Function of getting file size
fn size_from_file(path: &PathBuf) -> Result<u64, io::Error>{
	let file: File;
	let len: u64;
...
//Function of getting file bytes
fn bytes_from_file(path: &PathBuf) -> Result<Vec<u8>, io::Error>{
	let filecontent: Vec<u8>;
	match fs::read(path) {
		Ok(result) => filecontent = result,
		Err(error) => return Err(error)
